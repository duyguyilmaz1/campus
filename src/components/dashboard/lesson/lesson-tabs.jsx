...
const LessonTabs = () => {
  const [key, setKey] = useState("terms"); //burası da reactBootstrap sitesinden geldi
  const { currentOperation } = useSelector((state) => state.misc);

  // tabs yapısını reactBootstrap sitesinden alıp kopyaladık
  return (
    <Tabs
      ...
      onSelect={(k) => setKey(k)}
      className="mb-3"
      fill={true} //burayı true yapınca tablar ekrana yayılır, ekranı kaplar.
    >
      <Tab eventKey="terms" title="Education Terms">
        <Spacer height={30} />
        {currentOperation === "new" && ( //new butonuna basinca gorulcek
          <>
