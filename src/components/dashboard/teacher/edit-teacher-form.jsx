...
const EditTeacherForm = () => {
    ...
    const { currentRecord } = useSelector(state=> state.misc);  //ediltenen data merkezi state e teacher-list de gonderilmisti--> o datayi alabilemk icin merkezi state i cagirdik 

    const initialValues = {
        ...currentRecord, // burada editleyeceğimiz userın tüm bilgileri saklanıyor ki edite hazır olarak gelsin(edit butonuna basılınca bilgiler gelir)
        password:"",
        confirmPassword:"",
        lessonsIdList:[] //formik icinde yok o yuzden edit yapilan teacherin o an coktan secilmis olg dersleri gostermk icin eytra ekledik 
    }
    ...
    const validationSchema = Yup.object({
      ...
      const formik = useFormik({
        ...
        validationSchema,
        onSubmit,
        enableReinitialize: true //...currentRecord dan dolayi, currentRecord degistikce burasi da guncellencek 
      });
      ...
      const getTeacherPrograms = async () => { //edit yaparken teacher in database deki bilgilerini getirirken daha onceden atanmis ders prograindaki derslerin getirimeisni yani cehck edilmis sekilde gosterilmeisni saglar
        try {
          const data = await getTeacherById(currentRecord.userId); // editlenecek teacher id sini aldık
      ...
      useEffect(() => {
        getTeacherPrograms();
      }, [currentRecord]) // currentRecordda değişiklik olduğunda tekrar render eder yani tekrar calışır burası
